...
* [The Spotify model]() dispenses with Scrum of Scrums in favour of a
  ...
  practices, calling its smallest teams squads rather than scrums to avoid
  dictating Scrum rules. It rightly expects a structure of top-flight
  autonomous teams is capable of deciding for itself how to evolve the
  rollout of Agile practices, but it is committed to Continuous Delivery and
  DevOps with every tribe containing an Ops chapter dedicated to enabling
  squads to do CD.
...
It offers more than just a refactoring of what came before, however, adding:

* Continuous adaptation using the [Seven Samurai](http://agiletng.org/2014/03/21/the-seven-samurai/) pattern
  language.
* Continuous product planning using Lean Startup, Lean UX and [BPP](#feature-points)
* Continuous enterprise self-organization using [Iroquois
  councils](http://agiletng.org/2014/04/02/iroquois-council/)
...
the fundamentals of Agile technical excellence. These patterns interlock to assure
...
only refers to one of four Lean pathologies, muda. Lean also works to
minimize overburden (muri) and irregularity (mura). 

{% img /images/mmm.jpg %}

The maxim also admits an ambiguity. [Lean Startup](), [Lean UX]() and
...
We say net ROI to account for Cost of Delay (COD), and we represent both
...
This isn't only about products in their marketplace, but also the structure
and practices of squads in streams and streams in the enterprise. This
