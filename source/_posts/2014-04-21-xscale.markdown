...
**\[This post is currently in first draft - don't take it seriously yet. I
...
* [SAFe]() targets existing enterprises with a low-discipline, top-down,
  ...
  ignores the key enabler of scale-symmetry.
...
silos into feature teams. And like Spotify XSCALE uses Dunbar-number limits,
chapters, guilds, CD and DevOps.

XSCALE is more than a synthesis of the best aspects of these forbears. It
enables continuous adaptation using the [Seven Samurai]() patterns,
continuous product planning using Lean Startup, Lean UX and [BPP/BDD]()
patterns, and continuous enterprise self-organization using [Iroquois
councils](). Its intent is the greatest Agile benefit with the lowest risk
of disruption to existing value streams.
...
Yet golfers are untroubled by vast fields of grass blades.  They use just a
...
combinations of individual grass blades. 

Agile already uses this when it 
...
The Spotify / Iroquois patterns also do it by limiting the scope of
conversations to small teams with specific concerns.  And XSCALE takes the
same approach. The principle is:

**Iterate distinctions to control combinations**

A commonplace scaling challenge is the problem of determining which
...
To control this XSCALE uses the Behavioral Product Planning consensus game,
...
make decisions about alternatives at finer and finer scales. 
...
**Use consensus games to reach decisions and to delimit accountabilities.**
...
fragile. This is probably an artefact of the [anthropocene era]() where most
...
cycles of change to maintain stable behaviours over time. 

A living ecosystem differs from a jumble of creatures in a zoo in that it has
...
that don't efficiently adapt to the whole. 

As humans our advantage over other creatures is our ability to anticipate
...
* an enterrpise council composed of stream council representatives. 

At each level there are specific accountabilities defined by the teams.  Each
...
in Gherkin format by their respective councils.
...
that the entire enterprise pivots collaboratively to improve its ROI.
...
in terms of net ROI, and reconcile them with their avaialble funding levels. 
...
It does this by changing the unit of estimation from story points to **feature
...
no need to dream up a linear mapping between feature points and story points.
...
establishing a reasonable level of funding per feature. 

Backlog Bingo is very quick and easy to play:
...
Backlog Bingo can also be used without a dollar basis to estimate relative
...
#### Behavioral Product Planning

Behavioral Product Planning (BPP) is a simple consensus game that uses Backlog
...
uses Backlog Bingo to assign values to the BFR features in terms of:
...
uncertainty, about which see below.

The second bingo game reverses the rules. Now it's business stakeholders,
...
In any case, only technical team members can make effort estimates, and only
...
Once each feature has both numbers, prioritisation involves three steps:
...
* Divide the business value number, which represents expected return, by
  ...
  yields relative return on investment, which we call ROI.
* List features vertically in order of decreasing ROI. Let the PO increase the
...
* Once the PO is content with the prioritisation of business features, let
  ...
  position higher than the highest prioity feature that depends upon it.

Once these adjustments have been made, record the relative priorities of the
features to prepare them for Roadmap Refactoring, which determines how to best
fit them given funding constraints and MVP per objective. See details below.
...
estimable but another part is not:
...
If a large number of features are inestimable it's usually a good idea to
...
have sufficient time to resolve unsolved business questions.
...
simplifies to (Business Value / Dureation) + (Criticality / Duration).

There are several problems with WSJF:
...
-  WSJF doesn't account for technical CoD. It's completely blind to the
   ...
   concerns or business uncertainty. It just leaves that out.

All of these problems are eliminated simply by BPP. Once you've factored out
...
technical CoD. It's quick, easy and completely unambiguous.

#### Roadmap Refactoring

Roadmap Refactoring is a consensus game that leverages BPP to enable product
...
takes the prioritized feature
list, grouped by epic, costs 
