...
**\[This post is currently in draft. It covers most of the ground but the
graphics and some of the text still need polish. I'm sharing it as-is to
garner feedback. You can contact me directly or find a disqus box at the
bottom of this page. Or raise issues on the A:TNG github project if you
prefer. Refer to this draft as V0.5.\]**
...
* [SAFe]() targets existing enterprises with a low-discipline, top-down,
  ...
  from feature teams, continuous delivery and scale-symmetry.

* [LeSS]()/Agile Path/Enterprise Scrum, etc., are scale-symmetric
  alternatives closely based on traditional Scrum. They offer low-discipline
  high-collaboration, continuous cadence models that employ [Scrum of
  Scrums]() to deal with integration and cross-cutting concerns. XP
  practices are optional but feature teams are mandatory. LeSS is
  big-change-up-front but Agile Path is an iterative alternative.  Specific
  practices for product planning, test automation and so on are left for
  teams to determine ad hoc. 
...
* [The Spotify model]() dispenses with Scrum of Scrums in favour of a
  ...
  practices as it rightly expects a structure of top-flight autonomous teams
  is capable of deciding for themselves how to evolve the rollout of Agile
  practices, but it's committed to Continuous Delivery and DevOps with every
  tribe containing an Ops chapter dedicated to enabling squads to do CD.
  
XSCALE includes the best aspects of the older practices:

* Like SAFe, XSCALE standardizes team practices using Scrum-XP
* Like Agile Path, it transforms the enterprise iteratively
* Like LeSS, it refactors component silos into feature teams 
* Like Spotify it uses tribes, chapters, guilds, CD and DevOps

XSCALE offers more than just a refactoring of what came before, adding:

* Continuous adaptation using the [Seven Samurai]() pattern language.
* Continuous product planning using Lean Startup, Lean UX and [BPP]()
* Continuous enterprise self-organization using [Iroquois councils]()

But why these specific practices? XSCALE's intent is to afford enterprises
on all scales an absolute state of the art Agile capability while minimizing
risk of disruption to their existing value streams. So let's take a step back
and understand this in principle before we get down to nuts and bolts.

## XSCALE Principles

XSCALE conforms with the [A:TNG
values](http://agiletng.org/2014/03/10/4-new-values/), which form a strict
superset of those in the Agile Manifesto. XSCALE further assumes six
principles that also form a superset of the ones in the Manifesto.
...
Explanation for each of these follows, then the XSCALE practice patterns
that embody them.

### eXcellent

The term "extreme" in the original XP unfortunately evoked the idea of
recklessness, which wasn't helpful in the fledgling days of Agile.
Nevertheless the manifesto authors recognized its intent in a principle that
reads, "Continuous attention to technical excellence and good design
enhances agility". So XSCALE calls such continuous attention "eXcellent" and
sums it up using Beck's famous XP maxim:
...
of just 10 things has over 3.6 million distinct combinations. Yet golfers
are untroubled by combinations of grass blades. Golfers make simple
distinctions to control the combined behaviours of grass: "the green", "the
rough", and so on. They never think about combinations of the individual
blades. 
...
to small meetings with specific concerns. In general the principle is:
...
gets combinatorially hard. As agile works as a hill-climbing algorithm,
continously integrating little stories into a whole, it's quite happy to
climb a molehill of value when there's an Everest right next door.  

To control this combinatorial space XSCALE uses the Behavioral Product
Planning games, which make breadth-first distinctions about business
alternatives and then sort them into easily optimized piles. BPP iterates
...
able to iteratively break component silos into feature teams, and thereby
avoids these penalties because feature teams can use the GitFlow pattern
language for continuous delivery. The principle is simply:
